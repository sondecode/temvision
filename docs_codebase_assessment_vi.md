...
1. **(Đã xử lý) Hai nhánh vận hành LoL song song**
   - Trước đây có tách `main.py --game=lol` và `lol_overlay.py`.
   - Hiện tại đã hợp nhất về `main.py --game=lol` + `LoLModule`.
...
4. **(Đã xử lý) Khác biệt naming/config giữa generic và LoL chuyên sâu**
   - Đã hợp nhất config chính về `config/lol.yaml`.
